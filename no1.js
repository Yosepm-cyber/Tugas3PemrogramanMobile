...
let hasil
    ...
    if (umur > 40) {
        if (tinggal == "Nevada" || tinggal == "New York" || tinggal == "Havana") {
            if (tabungan > 1000000) {
                hasil = `${nama} memiliki kemungkinan seorang anggota mafia dengan pangkat ${pangkat[0]}`
            }else{
    ...
    //kondisi untuk Underboss    
        ...
        if (tinggal == "New Jersey" || tinggal == "Manhattan" || tinggal == "Nevada") {
            if (tabungan >= 1000000 && tabungan <= 2000000) {
                hasil = `${nama} memiliki kemungkinan seorang anggota mafia dengan pangkat ${pangkat[1]}`
            }else{
                hasil = `${nama} tidak mencurigakan`
            }
        }else{
    ...
    //kondisi untuk Capo
        ...
        if (tinggal == "California" || tinggal == "Detroit" || tinggal == "Boston" ) {
            if (tabungan < 1000000) {
                hasil = `${nama} memiliki kemungkinan seorang anggota mafia dengan pangkat ${pangkat[2]}`
            }else{
                hasil = `${nama} tidak mencurigakan`
            }
        }else{
